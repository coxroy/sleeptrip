...
% - calculates various detection statistics

%we continue from cfg_artifacts (as returned by st_run_detector_set)
cfg_artifacts.scoring=scoring; %the scoring is required to get statistics by stage

%field "segment_length" specifies segment length (in seconds)
%cfg_artifacts.segment_length=5; %this is the default (may be left out): length of segments (in sec)
cfg_artifacts.segmentrejectthresh=0.25;
cfg_artifacts.badchannelthresh=0.5;
...
%field "title" allows providing a title
cfg_artifacts.title='';
...
%In brief, cleaning proceeds by using the "grid_repair" matrix to determine, per segment, which channels should be interpolated.
%Segments marked for rejection are not cleaned.

%assign to data_clean so we keep both the raw and the cleaned data
data_clean=st_repair_artifacts(cfg_artifacts,data);
...
% scoring_artifact_level is the original scoring "resampled" to the segment length (default 5 s).
...
stage_data_excRej_excArt=st_select_data(cfg_select, data_clean); %cleaned data
