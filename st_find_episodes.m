...
episode_table= table(...
    ...
    'VariableNames',{...
    'episode_label','episode_num','start_epoch', 'end_epoch', 'dur_epochs'}...
    );

episode_table.start_seconds=(episode_table.start_epoch-1)*(scoring.epochlength); % -1 for START of epoch
episode_table.end_seconds=(episode_table.end_epoch)*(scoring.epochlength); %no -1 for END of epoch
    episode_table.dur_minutes=episode_table.dur_epochs*(scoring.epochlength/60);
